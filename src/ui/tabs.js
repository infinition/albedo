 ...
 *
 * This module is the strip and nothing else. The documents themselves — what is
 * in each scene, which of them is modified, what the engine did to it — live
 * where that state lives, and are handed here only as the four things a tab
 * needs to draw itself: an id, a title, whether it is modified, and whether it
 * is the active one.
 *
...
export function createTabs({ host, onActivate, onClose, onNew }) {
  host.textContent = "";
  ...
  function paint(docs, activeId) {
    ...
    for (const doc of docs) {
      ...
      tab.className = "tabs-tab" + (doc.id === activeId ? " active" : "");
      ...
      tab.title = doc.dirty ? `${doc.path || label} — modifié` : doc.path || label;
